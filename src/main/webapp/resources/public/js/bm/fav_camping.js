...
if(data.length===0) {
	...
	data.forEach(c => {
		...
		hearts.insertAdjacentHTML("afterbegin", html);
		document.querySelector(".list_summery").addEventListener("click", function(){
			window.location.href=`campDetail.do?contentid=${c.contentid}`
		})
		...
		document.querySelector(".list_text_overlay").addEventListener("click", function(){
			window.location.href=`campDetail.do?contentid=${c.contentid}`
		})
